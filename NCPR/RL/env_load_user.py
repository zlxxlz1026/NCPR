...
class LoadUserInfoEnv(object):
    def __init__(self, kg, dataset, data_name, seed=1, max_turn=15, cand_len_size=20, attr_num=20, command=1, ask_num=1, entropy_way='weight entropy', fm_epoch=250):
        ...
        self.ui_dict = self.__load_rl_data__(data_name)  # np.array [ u i weight]
    ...
    def __load_rl_data__(self, data_name):
    ...
    def reset(self, user_id):
        ...
        self.set_print_info('-----------reset state vector------------')
        # print('user_id:{}, target_item:{}'.format(self.user_id, self.target_item))
        self.set_print_info('user_id:{}, target_item:{}, item_feature:{}'.format(self.user_id, self.target_item, list(self.kg.G['item'][self.target_item]['belong_to'])))
        ...
        self.conversation_info.append(self.user_acc_feature[-1])
        self.cur_conver_step += 1

        # print('=== init user prefer feature: {}'.format(self.cur_node_set))
        self.set_print_info('=== init user prefer feature: {}'.format(self.cur_node_set))
        ...
        self.set_print_info('reset_reachable_feature num: {}'.format(len(self.reachable_feature)))
    ...
    def step(self, action):   #action:0  ask   action:1  recommend   setp=MAX_TURN  done
        ...
        self.set_print_info('---------------step:{}-------------'.format(self.cur_conver_step))
        if self.cur_conver_step == self.max_turn:
            ...
            self.set_print_info('--> Maximum number of turns reached !')
            done = 1
        elif action == 0:   #ask feature
            # print('-->action: ask features')
            self.set_print_info('-->action: ask features')
            ...
            if reward == 1:
                ...
                self.set_print_info('-->Recommend successfully!')
            else:
                if self.command in [1, 2, 6, 7]:  # update attr_ent
                    self._update_feature_entropy()
                # print('-->Recommend fail !')
                self.set_print_info('-->Recommend fail !')
    ...
    def _ask_update(self):
        ...
        self.set_print_info('-->choose feature: {}'.format(list(set(remove_acced_reachable_fea[:self.ask_num]))))
        ...
        if len(acc_feature):
            ...
            self.set_print_info('-->user feedback: accept!')
            self.conversation_info.append(acc_feature)
        else:
            reward = self.reward_dict['ask_fail']
            self.conver_his[self.cur_conver_step] = self.history_dict['ask_fail']  #update conver_his
            self.set_print_info('-->user feedback: reject!')
            self.conversation_info.append(rej_feature)
    ...
    def _update_cand_items(self, acc_feature, rej_feature):
        if len(acc_feature):    #accept feature
            ...
            # self.set_print_info('=== ask acc: update cand_items')
    ...
    def _recommend_updata(self):
        ...
        self.set_print_info('-->action: recommend items')
        recom_items = self.cand_items[: self.rec_num]    # TOP k item to recommend
        self.set_print_info('-->items list: {}'.format(list(recom_items)))
        if self.target_item in recom_items:
            reward = self.reward_dict['rec_suc']
            self.conversation_info.append(recom_items)
    ...
    def get_conversation_info(self):
        ...
        return self.conversation_info[self.cur_conver_step - 1]

    def get_conversation_his(self):
        return self.conver_his[self.cur_conver_step - 1]

    def set_print_info(self, msg):
        # self.print_info += msg + '\n'
        self.print_info.append(msg)
